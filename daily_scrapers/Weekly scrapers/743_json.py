...
df = pd.read_sql("""
           SELECT *
            FROM web_scraping_masters where status =  'Y' and source_name = 'Morningstar' and json_structure_type = 2 and source_indicator_name = 'Asset Class'
            """, con = mydb)
...
def fetch_data():
    ...
    for source, master__id in zip(src_list, master):
        ...
        try:
            ...
            json_url = "https://api-global.morningstar.com/sal-service/v1/etf/process/asset/v2/{}/data?locale=en&clientId=MDC&benchmarkId=category&version=3.31.0".format(morning_id)
            while True:
                try:
                    json_res = requests.get(json_url,headers=headers1).json()
                except Exception as e:
                    print(e)
                    sleep(2)
                    json_res = requests.get(json_url,headers=headers1).json()
            ...
            name = ['Equity','Convertible','Fixed income','Cash','Other','Preferred']
            for name_id,key in zip(name,json_res['allocationMap']):
                total_list.append({"name":key.replace('INDAssetAlloc','').replace('AssetAlloc','') ,"Net_Assets_%":str(json_res['allocationMap'][key]['netAllocation'])})
            # print(total_list)
            main_map[master__id]["indicators"]["743"] = {"value_data":0,"json_data":total_list}       
