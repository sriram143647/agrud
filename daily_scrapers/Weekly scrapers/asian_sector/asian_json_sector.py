...
df = pd.read_sql("""
                ...
                WHERE indicator_id = '742' AND source_name = 'Morningstar' AND (exchange_id = '10' OR exchange_id = '12' OR exchange_id = '11') 
            """, con = mydb)
...
df_ = indicator.loc[:, ~indicator.columns.duplicated()]
source_map = {}
...
json_format = []
for size in df['json_structure_type']:
    json_format.append(size)

def fetch_data():
  
    main_map = {}
    for source, master__id,forma in zip(src_list, master,json_format):
        ...
        try:
            ...
            new_dict = funcy.omit(response['FIXEDINCOME']['fundPortfolio'], ('portfolioDate'))
            new_dict1 = funcy.omit(response['EQUITY']['fundPortfolio'], ('portfolioDate'))
            total_list = []
            if forma == 1:
                for key,value in new_dict.items():
                    # print({key:value})
                    dict =  {"name":re.sub(r"(\w)([A-Z])", r"\1 \2", str(key)).title() , "Equity_%":str(value)}
